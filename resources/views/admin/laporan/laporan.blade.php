        ...
        <table class="table table-hover mb-3" id="table">
            ...
            <tbody>
                ...
                @foreach ($report as $number => $item)
                    @php
                        $totalByDate[$item->tgl_pesanan] = ($totalByDate[$item->tgl_pesanan] ?? 0) + $item->total_harga;
                    @endphp
                @endforeach

                @foreach ($report as $number => $item)
                    @if (!in_array($item->tgl_pesanan, $uniqueDates))
                        @php
                            $uniqueDates[] = $item->tgl_pesanan;
                        @endphp
                        <tr>
                            <td class="">{{ count($uniqueDates) }}</td>
                            <td class="">{{ $item->tgl_pesanan }}</td>
                            <td class="">Rp. {{ number_format($totalByDate[$item->tgl_pesanan]) }}</td>
                            <td class="" hidden>
